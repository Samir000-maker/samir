...
let viewedItemsSkipped = 0;
let excludedByClient = 0; // ✅ ADD THIS

// ✅ CRITICAL FIX: Create COMBINED exclusion set
const combinedExclusionSet = new Set([...excludedIds, ...viewedIds]);
console.log(`[PHASE-5-EXCLUSIONS] Total exclusions: ${combinedExclusionSet.size} (client: ${excludedIds.length}, viewed: ${viewedIds.size})`);

// ✅ STEP 1: Collect ALL content FIRST, then filter BOTH viewed AND excluded
for (const { slotId, content } of slotContents) {
  totalItemsBeforeFilter += content.length;
  
  for (const item of content) {
    // ✅ CRITICAL FIX: Check COMBINED exclusion set
    const isExcluded = combinedExclusionSet.has(item.postId);
    ...
    if (isExcluded) {
      if (viewedIds.has(item.postId)) {
        ...
        excludedByClient++;
        console.log(`[FILTER-CLIENT-EXCLUDED] Skipping ${item.postId.substring(0, 8)} - in client exclusion list`);
      }
...
console.log(`[PHASE-5-FILTERING-SUMMARY] Total: ${totalItemsBeforeFilter} | Viewed: ${viewedItemsSkipped} | Client-excluded: ${excludedByClient} | Remaining: ${allContent.length}`);

// ✅ STEP 2: Sort by priority (unchanged)
allContent.sort((a, b) => {
...
// ✅ STEP 3: Apply content criteria logic (unchanged)
let selectedContent = [];
...
const interestedContent = selectedContent;
...
console.log(`[PHASE-6-STATS] Content ready: ${interestedContent.length} items`);
console.log(`[PHASE-6-STATS] Total excluded: ${viewedItemsSkipped + excludedByClient} (viewed: ${viewedItemsSkipped}, client: ${excludedByClient})`);

// ✅ Sanity check - verify no excluded items leaked through
const leakedItems = interestedContent.filter(item => combinedExclusionSet.has(item.postId));
if (leakedItems.length > 0) {
  console.error(`🚨 [CRITICAL-BUG] ${leakedItems.length} excluded items leaked through! IDs: ${leakedItems.map(i => i.postId.substring(0, 8)).join(', ')}`);
} else {
  console.log(`✅ [VERIFICATION-PASSED] No excluded items found in final content`);
}
...
console.log(`  Excluded: ${viewedItemsSkipped + excludedByClient} items (viewed: ${viewedItemsSkipped}, client: ${excludedByClient})`);
...
return {
  content: finalContent.slice(0, limit), // ✅ Already fully filtered
  isNewUser,
  ...
  metadata: {
    ...
    excludedByViewed: viewedItemsSkipped,
    excludedByClient: excludedByClient, // ✅ ADD THIS
    totalExcluded: viewedItemsSkipped + excludedByClient, // ✅ ADD THIS
