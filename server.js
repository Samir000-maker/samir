...
const [userContribDoc, followResponse] = await Promise.all([
  // FIXED: Single query to get user's contribution document
  db.collection('contrib_posts').findOne({ _id: userId }, { projection: { contributions: 1 } }),
...
// FIXED: Extract contributed postIds from new structure
const serverViewedContent = [];

if (userContribDoc && userContribDoc.contributions) {
  // Extract all postIds from contributions object
  const contributedPostIds = Object.keys(userContribDoc.contributions);
  serverViewedContent.push(...contributedPostIds);
  
  console.log(`[METADATA-COMPLETE] Total viewed: ${contributedPostIds.length} content items from user doc`);
} else {
  console.log(`[METADATA-COMPLETE] No contributions found for user ${userId}`);
}

// Merge with excluded IDs from request
const allExcludedPosts = [...new Set([...serverViewedContent, ...(excludedPostIds || [])])];
const allExcludedReels = [...new Set([...serverViewedContent, ...(excludedReelIds || [])])];
